...
public class SensorTemperaturaController {
    ...
    @GetMapping("/")
    public ResponseEntity<List<SensorTemperatura>> listarTodos() {
        List<SensorTemperatura> sensores = sensorService.listarTodos();
        return ResponseEntity.ok(sensores);
    }

    @PostMapping("/")
    public ResponseEntity<SensorTemperatura> criarSensor(@RequestBody @Valid SensorTemperatura sensor) {
        SensorTemperatura novoSensor = sensorService.salvar(sensor);
